...
export const useTransferStore = defineStore('transfer', () => {
  ...
  const canStartNewTask = computed(
    () => activeTasks.value.size < maxConcurrentTasks.value,
  );
  ...
  const overallProgressPercentage = computed(() => {
    ...
    return Math.round(
      (totalTransferredSize.value / totalTransferSize.value) *
   ...
   * 开始缓存根目录迁移
   */
  async function startRootMigration(
  ...
  async function pauseTransfer(taskId: string): Promise<void> {
    try {
      ...
      handleStoreError(error, '暂停传输', (msg) => {
        lastError.value = msg;
      });
      throw error;
    }
  ...
  async function cancelTransfer(taskId: string): Promise<void> {
    try {
      ...
      handleStoreError(error, '取消传输', (msg) => {
        lastError.value = msg;
      });
      throw error;
    }
  ...
  async function resumeTransfer(taskId: string): Promise<void> {
    try {
      ...
      handleStoreError(error, '恢复传输', (msg) => {
        lastError.value = msg;
      });
      throw error;
    }
    ...
    if (task) {
      ...
      if (
        ...
        (updatedProgress.status === 'completed' ||
          ...
          updatedProgress.status === 'cancelled')
      ) {
        // 如果是剪切操作且成功完成，更新缓存列表并清理数据库
        if (
          updatedProgress.status === 'completed' &&
          task.operation === 'Cut'
        ) {
          handleCutOperationCompleted(task).catch(console.error);
        }
  ...
  function validateProgressData(
    ...
    if (previous && current.completedFiles < previous.completedFiles) {
      console.warn(
        `文件计数倒退: ${previous.completedFiles} -> ${current.completedFiles}`,
      );
    ...
    if (
      previous &&
      current.transferredSize < previous.transferredSize &&
      current.status !== 'pending'
    ) {
      console.warn(
        `传输大小倒退: ${previous.transferredSize} -> ${current.transferredSize}`,
      );
    ...
    if (validated.completedFiles > validated.totalFiles) {
      console.warn(
        `已完成文件数超过总数: ${validated.completedFiles} > ${validated.totalFiles}`,
      );
    ...
    if (validated.transferredSize > validated.totalSize) {
      console.warn(
        `已传输大小超过总大小: ${validated.transferredSize} > ${validated.totalSize}`,
      );
    ...
    if (validated.totalSize > 0) {
      const calculatedPercentage =
        (validated.transferredSize / validated.totalSize) * 100;
      ...
      validated.percentage =
        validated.completedFiles >= validated.totalFiles ? 100 : 0;
    }
    ...
    // 处理当前文件信息缺失的情况
    if (
      !validated.currentVideoName ||
      validated.currentVideoName.trim() === ''
    ) {
   ...
   * 处理剪切操作完成
   */
  async function handleCutOperationCompleted(
    ...
    try {
      ...
      for (const sourcePath of task.source_files) {
        const itemToRemove = cacheStore.cacheItems.find(
          (item: CacheTypes.CacheItem) => item.cachePath === sourcePath,
        );

        if (itemToRemove) {
          itemIdsToDelete.push(itemToRemove.id);

          // 从选中项目中移除
          const selectedIndex = cacheStore.selectedItems.indexOf(
            itemToRemove.id,
          );
          if (selectedIndex !== -1) {
          ...
          // 从显示项列表中移除
          cacheStore.displayItems = cacheStore.displayItems.filter(
            (displayItem: CacheTypes.DisplayItem) => {
      ...
      // 清理数据库中的记录（文件已被移动，只删除数据库记录）
      for (const id of itemIdsToDelete) {
        try {
          await backend.commands.deleteCacheItem(id);
        } catch (error) {
          console.warn(`清理数据库记录失败 ${id}:`, error);
        }
      }
    ...
    try {
      ...
      channel.onmessage = (backendProgress) => {
        ...
        const progress: Types.TransferProgress = migrateTransferProgress({
          ...
          status:
            backendProgress.status.toLowerCase() as Types.TransferTaskStatus,
        });
  ...
  async function refreshTaskProgress(
    ...
    try {
      const backendProgress =
        await backend.commands.getTransferProgress(taskId);
      if (backendProgress.status === 'ok' && backendProgress.data) {
        ...
        const progress: Types.TransferProgress = migrateTransferProgress({
          ...
          status:
            backendProgress.data.status.toLowerCase() as Types.TransferTaskStatus,
        });

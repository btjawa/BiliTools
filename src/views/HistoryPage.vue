...
<div class="flex w-full h-full mt-[22px] flex-1 gap-3 min-h-0">
	...
	<VList
		v-if="v.listActive" #default="{ item }"
		...
		>
			<div class="relative flex rounded-lg min-w-40 overflow-hidden">
				<Image
					...
					:height="96" :width="160" :ratio="5/3" v-model="coverCache"
				/>
				<div class="absolute w-full h-full z-10 bg-linear-to-b from-transparent to-black/50"></div>
				<ProgressBar
					:progress="item.progress === -1 ? 100 : (item.progress / item.duration || 0) * 100"
			...
			<div class="w-full flex flex-col gap-1 min-w-0">
				...
				<div class="desc" v-if="item.duration">
					...
					<span>{{ duration(item.progress === -1 ? item.duration : item.progress) }}/{{ duration(item.duration) }}</span>
				</div>
			</div>
			<a class="text-xs text-nowrap mb-auto"
				@click="openUrl('https://space.bilibili.com/' + item.author_mid)"
			>{{ item.author_name }}</a>
			<button
				class="absolute right-3 bottom-3"
				v-if="item.videos" @click="search(item)"
			>
...
<div class="popup" :class="{ 'active': v.filter }">
	...
	<div class="gap-4 pr-16" v-if="v.filter">
		...
		<div v-for="(val, k) in filters" class="flex gap-2 *:flex-shrink-0">
			<button
				v-for="i in val" @click="v.select[k] = i as any"
...
import { inject, onActivated, reactive, Ref, ref, Transition, watch } from 'vue';
...
watch(() => v.range, () => v.select.time = 'custom');
...
function apply() {
	...
	const time = {
		...
		week: [new Date().setHours(-((new Date().getDay() + 1) * 24),0,0,0) / 1000,0],
		custom: [

...
export function formatFileSize(bytes: number, precision?: number): string {
  ...
  // 根据大小调整精度，支持自定义精度
  const finalPrecision = precision !== undefined ? precision : (sizeIndex === 0 ? 0 : FILE_SIZE.DECIMAL_PLACES);
  
  // 对于小数值，避免显示不必要的小数位
  const formattedValue = sizeIndex === 0 || value >= 100 
 ...
 */
export function formatSpeed(bytesPerSecond: number, precision?: number): string {
 ...
 * @returns 格式化后的时间字符串
 */
export function formatTime(seconds: number, showSeconds: boolean = true): string {
  ...
  if (days > 0) {
    return hours > 0 ? `${days}天${hours}${TIME_FORMAT.UNITS.HOURS}` : `${days}天`;
  } else if (hours > 0) {
    return minutes > 0 ? `${hours}${TIME_FORMAT.UNITS.HOURS}${minutes}${TIME_FORMAT.UNITS.MINUTES}` : `${hours}${TIME_FORMAT.UNITS.HOURS}`;
  } else if (minutes > 0) {
 ...
 */
export function formatSizeProgress(transferred: number, total: number, precision?: number): string {
